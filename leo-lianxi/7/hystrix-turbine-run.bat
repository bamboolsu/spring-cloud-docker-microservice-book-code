...
echo XXXXXXXX
echo ３，　启动　microservice-consumer-movie-ribbon-hystrix
...
echo XXXXXXXX
echo ４，　启动　microservice-consumer-movie-ribbon-hystrix
echo http://localhost:8020/user/1      http://localhost:8020/log-user-instance
echo XXXXXXXX spring-cloud-starter-netflix-eureka-client  
echo XXXXXXXX  spring-cloud-starter-netflix-hystrix
cd ../../microservice-consumer-movie-feign-hystrix-fallback-stream/target/
echo  start java -jar  microservice-consumer-movie-feign-hystrix-fallback-stream-0.0.1-SNAPSHOT.jar   --server.port=8020
start java -jar  microservice-consumer-movie-feign-hystrix-fallback-stream-0.0.1-SNAPSHOT.jar   --server.port=8020
echo XXXXXXXX 

echo XXXXXXXX
echo 5，　启动　microservice-hystrix-turbine
echo http://localhost:8031/turbine.stream   
echo XXXXXXXX spring-cloud-starter-netflix-eureka-client  
echo XXXXXXXX  spring-cloud-starter-netflix-hystrix
cd ../../microservice-hystrix-turbine/target/
echo  start java -jar  microservice-hystrix-turbine-0.0.1-SNAPSHOT.jar   --server.port=8031
start java -jar  microservice-hystrix-turbine-0.0.1-SNAPSHOT.jar   --server.port=8031
echo XXXXXXXX 


echo XXXXXXXX
echo 6，　启动　microservice-hystrix-dashboard
echo http://localhost:8030/dashboard 
echo XXXXXXXX spring-cloud-starter-netflix-eureka-client  
echo XXXXXXXX  spring-cloud-starter-netflix-hystrix
...
echo XXXXXXXX 
echo  7： 访问 http://localhost:8010/user/1
echo XXXXXXXX

echo XXXXXXXX 
echo  8： 访问 http://localhost:8020/user/1
echo XXXXXXXX

echo XXXXXXXX 
echo  9： 访问 http://localhost:8030/hystrix.stream    hystrix的监控； 在地址栏里输入　http://localhost:8031/turbine.stream
echo XXXXXXXX
